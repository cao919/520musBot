...
async function onMessage(msg) {
    ...
    if (room &&isText) { // 如果是群消息
        ...
        console.log(`处理1发消息人: ${alias} 消息内容: ${content}`)
        if(content.substr(0,1)=='?'||content.substr(0,1)=='？'){
            let contactContent = content.replace('?','').replace('？','')
            if(contactContent){ 
                console.log(`处理2发消息人: ${alias} 消息内容: ${content}`)
                let res = await superagent.getRubbishType(contactContent)
                await delay(2000)
                await contact.say(res)
            }
        } 
        else  if(config.AUTOREPLY&& config.AUTOREPLYPERSONS.indexOf(alias)>-1)
        {
            console.log(`处理3发消息人: ${alias} 消息内容: ${content}`)
            ...
            if (content&&alias!='A朵老婆大人'&&alias!='春天的脚步') { 
                ...
                console.log('520mus回复：', reply)
                let str520MUS= '520mus回复:'+reply
                try {
                    await delay(2000)
...
async function initDay() {
    ...
    schedule.setSchedule(config.SENDDATEsecond, async() => {
...
async function initDay1() {
    ...
    schedule.setSchedule(config.SENDDATEsecond1, async() => {
        ...
        let str1 = today1  + '<br>很晚了,要睡美美的觉了噢^_^<br>'  + '<br>每日土味情话：<br>' + sweetWord1 + '<br><br>' + '————————最爱你的我'
...
async function initDay2() {
    ...
    schedule.setSchedule(config.SENDDATE2, async() => {
        ...
        let str2 = today2  + '<br>很晚了,不能通宵噢^_^<br>'  + '<br>每日土味情话：<br>' + sweetWord2 + '<br><br>' + '————————最爱你的我'
